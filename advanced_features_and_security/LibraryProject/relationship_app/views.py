...
def home(request):
    if request.user.is_authenticated:
        return HttpResponse(f"Welcome {request.user.alias_name} to the Library Home Page!") # A simple home page response
...
# Set permissions
# Get the permission
permission = Permission.objects.get(codename='add post')

# Check if user has permissions
def staff_view(request):
    if request.user.has_perm('relationship_app.add_post'):
        # Grant user access
        return "You have 'create post' privileges"
    else:
